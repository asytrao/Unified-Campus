...
class ContentSubmissionStatusPage extends StatelessWidget {
  ...
  void _openFile(BuildContext context, String url) async {
    // Debug logging
    print("Opening file with URL: $url");

    if (url.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text("⚠️ No file URL available")));
      return;
    }
    ...
    try {
      if (url.toLowerCase().endsWith('.pdf')) {
        ...
        // For other file types, try to open in external browser
        ...
        if (await canLaunchUrl(uri)) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("⚠️ Cannot open file. URL: $url"),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("⚠️ Error opening file: $e"),
          duration: const Duration(seconds: 5),
        ),
      );
    }
    ...
    try {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          setState(() {
            _error =
                "URL not accessible (Status: ${response.statusCode})\nURL: ${widget.pdfUrl}";
            _isLoading = false;
          });
          return;
        }
        ...
        // Continue anyway, let the PDF viewer try
      }
